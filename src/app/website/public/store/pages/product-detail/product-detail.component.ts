...
export class ProductDetailComponent implements OnInit {
  ...
  sizeGuideTab: 'footwear' | 'clothing' = 'footwear';
  showLightbox: boolean = false;

  // Lista de deseos
  isWishlisted: boolean = false;

  // Acordeones colapsables para especificaciones y políticas
  ...
  // Barra fija inferior en móvil (Sticky Mobile Action Bar)
  ...
  selectColor(code: string): void {
    if (this.selectedColorCode === code) {
      ...
      if (firstItem) {
        this.selectedImage = firstItem.url;
        this.sliderIndex = 0;
      }
      return;
    }
  ...
  /** Monto en 3 cuotas sin intereses */
  get installmentAmount(): number {
    return this.finalPrice > 0 ? this.finalPrice / 3 : 0;
  }
  ...
  get availableStock(): number {
    ...
    const stores = (this.selectedVariant as any).storeAvailability;
    if (Array.isArray(stores) && stores.length > 0) {
      return stores.reduce((acc: number, s: any) => acc + (s.availableQty ?? 0), 0);
    }
    return 12; // Estimado por defecto si no hay geo
  }

  get isLowStock(): boolean {
    return this.selectedVariant !== null && this.availableStock > 0 && this.availableStock <= 5;
  }
  ...
  get primaryCategory(): string {
    return this.product?.category?.[0] ?? 'Catálogo';
  }
  ...
  getWarrantyTypeLabel(type: string | undefined): string {
    if (!type) return 'Garantía Oficial';
    const map: Record<string, string> = {
      manufacturer: 'Garantía de Fábrica',
      store: 'Garantía de Tienda',
      none: 'Sin Garantía',
    };
    return map[type] || 'Garantía Oficial';
  }

  toggleAccordion(key: 'description' | 'specifications' | 'warranty' | 'shipping'): void {
  ...
  toggleWishlist(): void {
    ...
    if (this.isWishlisted) {
      this.toastService.showSuccess('Guardado en tu lista de deseos');
    } else {
      this.toastService.showInfo('Eliminado de tu lista de deseos');
    }
  }
  ...
  shareProduct(): void {
    ...
    if (navigator.share) {
      navigator
        .share({
          title: this.product?.name ?? 'Moorea',
          text: `Descubre ${this.product?.name} en Moorea`,
          url,
        })
        .catch(() => {});
      ...
      navigator.clipboard
        ...
        .catch(() => {
          this.toastService.showInfo('Enlace listo');
        });
    }
  }

  openSizeGuide(): void {
    const isShoes = this.product?.category?.some((c) => /calzado|zapatilla|zapato/i.test(c));
    this.sizeGuideTab = isShoes ? 'footwear' : 'clothing';
